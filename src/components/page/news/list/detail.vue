    ...
    <div class="context-box" :style="{'height': `${areaHeight}px`}" ref="context">
      <div v-show="activeIndex === 1">
        <el-form :model="model" ref="model" label-position="top" size="small">
          <el-form-item v-if="!$route.query.id" label="分类" prop="categoryId" :rules="[{ required: true, message: '该字段不能为空'}]">
          ...
          <el-form-item v-if="!$route.query.id" label="城市" prop="areaId" :rules="[{ required: true, message: '该字段不能为空'}]">
            <el-cascader size="small" :options="areaList" v-model="cityDefault" :show-all-levels="false" @change="cityChange"></el-cascader>
          </el-form-item>
          <el-form-item label="称呼" prop="appellation" :rules="[{ required: true, message: '该字段不能为空'}]">
            <el-input size="small" v-model.trim="model.appellation" style="width:210px;"></el-input>
          </el-form-item>
          <el-form-item label="手机" prop="phone" :rules="[{ required: true, message: '该字段不能为空'}]">
            <el-input size="small" v-model="model.phone" maxlength="11" style="width:210px;"></el-input>
          </el-form-item>
          ...
          <el-form-item label="内容" prop="content" :rules="[{ required: true, message: '该字段不能为空'}]">
            <el-input size="small" type="textarea" v-model="model.content" style="width:400px;"></el-input>
          </el-form-item>
          ...
          <!-- <el-form-item>
            <el-switch v-model="model.isHot" active-text="置顶" ></el-switch>
          </el-form-item>
          <el-form-item label="内容" prop="content" :rules="[{ required: true, message: '该字段不能为空'}]">
...
export default {
  ...
  methods: {
    ...
    apiGetNewsDetail() {
      api.getNewsDetail({ id: this.$route.query.id }).then(res => {
    ...
    submit() {
      ...
      this.$refs.model.validate(valid => {
        if (valid) {
          this.loading = true;
          if (this.$route.query.copy) this.addNews(model);
          else if (this.$route.query.id) this.updateNews(model);
          else this.addNews(model);
        } else {
